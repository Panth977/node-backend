...
function createSchemaCode(
    ...
    dependency: ($ref: string) => string
): { decorator: string; code: string } {
    ...
    if (schema.anyOf) {
        if (schema.anyOf.length === 1) {
            return createSchemaCode(context, schema.anyOf[0], dependency);
        }
        if (!schema.anyOf.length) {
            ...
            for (const possibleSchema of schema.anyOf) {
                const possibleSchemaCode = createSchemaCode(context, possibleSchema, dependency);
                code += `${possibleSchemaCode.decorator} ${possibleSchemaCode.code},`;
            }
            outputCode = `z.union([${code}])`;
        }
    } else if (schema.type === 'array') {
        const itemSchemaCode = createSchemaCode(context, schema.items, dependency);
        ...
        for (const caseName in schema.discriminator.mapping) {
            const caseSchemaCode = createSchemaCode(context, { $ref: schema.discriminator.mapping[caseName] }, dependency);
        ...
        const valueSchemaCode = createSchemaCode(context, schema.additionalProperties, dependency);
        const code = `${valueSchemaCode.decorator} ${valueSchemaCode.code}`;
        outputCode = `z.record(${code})`;
    } else if (schema.type === 'object') {
        let code = '';
        for (const propName in schema.properties) {
            const propSchemaCode = createSchemaCode(context, schema.properties[propName], dependency);
...
function createRouteCode(
    ...
    path: string,
    method: Method,
    route: OperationObject | undefined,
    dependency: ($ref: string) => string
...
export function typescript(context: Context, json: OpenAPIObject) {
    ...
    const dependencyCreated: Record<string, string> = {};
    function dependency($ref: string): string {
        if ($ref in dependencyCreated === false) {
            const schema = createSchemaCode(context, json.components?.schemas?.[$ref.substring($ref.lastIndexOf('/') + 1)], dependency);
            const name = genVarName();
            code += `${schema.decorator} const ${name} = ${schema.code};`;
            dependencyCreated[$ref] = name;
        }
        return dependencyCreated[$ref];
    }
    code += (function () {
        let routesCode = '';
        ...
        for (const path in json.paths) {
            for (const method in json.paths[path]) {
                if (
                    ...
                    const route = createRouteCode(context, path, method, json.paths[path][method], dependency);
                    const name = genVarName();
                    routesCode += `${route.decorator} const ${name} = ${route.code};`;
                    bundleCodes.default += `${JSON.stringify(route.name)}: ${name},`;
                    for (const tag of route.tags) {
                        bundleCodes[tag] ??= '';
                        bundleCodes[tag] += `${JSON.stringify(route.name)}: ${name},`;
                    }
                }
            }
        }
